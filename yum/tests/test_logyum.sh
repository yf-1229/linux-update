#!/usr/bin/env bash
# yum/tests/test_logyum.sh

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
ROOT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
SCRIPT="$ROOT_DIR/logyum"
...
if [ -x "$SCRIPT" ]; then
    pass "logyum is present and executable"
else
    fail "Script is missing or not executable: $SCRIPT"
fi
assert_exit "logyum --help exits 0" 0 bash "$SCRIPT" --help
...
assert_output_contains "logyum update runs yum update" "MOCK yum update" \
    env PATH="$MOCK_BIN_DIR:$PATH" LOGYUM_LOG_DIR="$TMP_LOG_DIR" bash "$SCRIPT" update
assert_output_contains "logyum install runs yum install" "MOCK yum install curl" \
    env PATH="$MOCK_BIN_DIR:$PATH" LOGYUM_LOG_DIR="$TMP_LOG_DIR" bash "$SCRIPT" install curl
assert_output_contains "logyum install forwards multiple packages" "MOCK yum install curl git" \
